...
class DQN():
	...
	def create_Q_network(self):
		...
		# Q Value layer
		#value
		W21 = self.weight_variable([self.hide_layer_inputs,1])
		b21 = self.bias_variable([1])
		self.V = tf.matmul(h_layer, W21) + b21

		#advantage
		W22 = self.weight_variable([self.hide_layer_inputs,self.action_dim])
		b22 = self.bias_variable([self.action_dim])
		self.A = tf.matmul(h_layer, W22) + b22
		self.Q_value = self.V + (self.A - tf.reduce_mean(self.A, axis=1, keep_dims=True))
		#save the weights
		self.Q_Weihgts = [W1,b1,W21,b21,W22,b22]
	...
	def create_TargetQ_network(self):
		...
		self.A = tf.matmul(h_layer, W22) + b22
		#save the weights
		self.TargetQ_value = self.V + (self.A - tf.reduce_mean(self.A, axis=1, keep_dims=True))
		self.TargetQ_Weights = [W1,b1,W21,b21,W22,b22]
...
def main():
	...
	for episode in range(EPISODE):
		...
		if episode % 100 == 99:
			...
			if episode % 1000 == 99:
				np.save('epi_duel_dqn.npy', np.array(epi))
				np.save('reward_epi_duel_dqn.npy', np.array(reward_epi))
			#if ave_reward >= 990:
			#	break
